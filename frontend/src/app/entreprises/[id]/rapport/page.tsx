...
export default async function DailyReportPage({
  ...
  const [res, companyRes] = await Promise.all([
    apiFetch(`/companies/${id}/reports/daily`),
    ...
    return (
      <Card tone="danger">
        <p className="text-sm font-medium">
          Impossible de charger le rapport du jour.
        </p>
        <p className="mt-1 text-sm opacity-90">
          Le serveur n&apos;a pas répondu : cet écran ne dit pas qu&apos;aucun
          rapport n&apos;existe pour cette période, il n&apos;a pas pu le
          récupérer.{" "}
          <a href={`/entreprises/${id}/rapport`} className="underline">
            Recharger la page
          </a>
  ...
  if (!report) {
    return <EmptyState>Aucun rapport disponible pour cette période.</EmptyState>;
  }
